...
# Assuming agreement_parser_v8.py is in the same directory or PYTHONPATH.
try:
    from agreement_parser_v8 import (
        AgreementParserV8Enhanced,
    ...
    print(f'Error importing from agreement_parser_v8: {e}')
    print('Please ensure agreement_parser_v8.py is in the Python path (e.g., current directory).')
    sys.exit(1)
...
def generate_visual_trees():
    ...
    html_files = sorted(input_dir.glob('agreement_*.html'))
    if not html_files:
