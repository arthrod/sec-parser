...
"""
    ...
    for case in sorted(degradations['any_degradation'], key=lambda x: x['agreement']):
    ...
    if degradations['more_orphans']:
        ...
        for case in sorted(degradations['more_orphans'], key=lambda x: x['increase'], reverse=True):
    ...
    if degradations['fewer_elements']:
        ...
        for case in sorted(degradations['fewer_elements'], key=lambda x: x['reduction'], reverse=True):
...
""".format(
        ...
        orphan_degradations=orphan_degradations,
        status_degradations=status_degradations,
        element_degradations=element_degradations
    )
...
def main():
    ...
    print("Analyzing ALL V8 parsing quality degradations...")
    print("=" * 80)
    
    degradations = analyze_all_degradations()
    
    print(f"Found degradations in {len(degradations['any_degradation'])}/100 agreements:")
    print(f"  - More orphans: {len(degradations['more_orphans'])} cases")
    print(f"  - Worse status: {len(degradations['worse_status'])} cases")
    print(f"  - Fewer elements: {len(degradations['fewer_elements'])} cases")
    
    # Generate comprehensive report
    report = generate_comprehensive_degradation_report(degradations)
    
    with open('v8_complete_degradation_analysis.md', 'w') as f:
        f.write(report)
    
    print(f"\nComplete degradation analysis saved to: v8_complete_degradation_analysis.md")
