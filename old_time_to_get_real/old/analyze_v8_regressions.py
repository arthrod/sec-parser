...
from agreement_parser_v7 import AgreementParserV7, analyze_agreement_v7, HierarchicalElement as V7HierarchicalElement
from agreement_parser_v8 import AgreementParserV8, analyze_agreement_v8, HierarchicalElement as V8HierarchicalElement
...
def main():
    ...
    print("Analyzing V8 Regression Cases...")
    print("=" * 80)
    ...
    print(f"Found {len(regression_cases)} regression cases:")
    for case in regression_cases:
        print(f"  Agreement {case['agreement_num']:03d}: {case['v7_orphans']} → {case['v8_orphans']} (+{case['increase']})")
    
    # Analyze the most significant regressions
    significant_regressions = [case for case in regression_cases if case['increase'] >= 5]
    print(f"\nAnalyzing {len(significant_regressions)} significant regressions (increase ≥ 5 orphans)...")
    
    detailed_analysis = []
    for case in significant_regressions:
        print(f"\nAnalyzing agreement {case['agreement_num']:03d}...")
        try:
    ...
    with open('v8_regression_analysis_report.md', 'w') as f:
        f.write(report_content)
    
    print(f"\nDetailed regression analysis saved to: v8_regression_analysis_report.md")
...
"""
    ...
    for case in sorted(all_regressions, key=lambda x: x['increase'], reverse=True):
