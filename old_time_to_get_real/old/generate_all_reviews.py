...
def analyze_single_file(agreement_num, html_dir, json_dir):
    ...
    try:
        ...
        print(f"Error analyzing agreement {agreement_num:03d}: {e}")
        return None
...
def generate_batch_summary(batch_num, analyses):
...
"""
    ...
    top_element_types = sorted(all_element_types.items(), key=lambda x: x[1], reverse=True)[:3]
...
def main():
    ...
    print("Generating all 10 batch review files...")
    
    for batch_num in range(1, 11):
        start_num = (batch_num - 1) * 10 + 1
        end_num = batch_num * 10
        
        print(f"Processing Batch {batch_num:02d} (Files {start_num:03d}-{end_num:03d})...")
...
"""
    ...
    print("\n✅ All 10 batch review files generated successfully!")
    print("\nFiles created:")
    for i in range(1, 11):
        print(f"  - review_batch_{i:02d}.md")
