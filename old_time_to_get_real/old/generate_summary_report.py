...
def generate_summary_report():
    ...
    print(f"Analyzing {len(json_files)} parsed agreements...")
    ...
    report_lines.append(f"Dataset: 100 random SEC material contracts from HuggingFace")
    report_lines.append(f"Parser: Edgar10QParser (standard sec-parser)")
    report_lines.append("")
    ...
    if large_files:
        for file_info in sorted(large_files, key=lambda x: x['elements'], reverse=True)[:5]:
            report_lines.append(f"  - {file_info['name']}: {file_info['elements']} elements")
    report_lines.append("")
    
    report_lines.append(f"Small files (<20 elements): {len(small_files)}")
    if small_files:
        for file_info in sorted(small_files, key=lambda x: x['elements'])[:5]:
            report_lines.append(f"  - {file_info['name']}: {file_info['elements']} elements")
    report_lines.append("")
    ...
    report_lines.append("-" * 50)
    for file_info in sorted(clean_files, key=lambda x: x['elements'], reverse=True):
        report_lines.append(f"✓ {file_info['name']}: {file_info['elements']} elements")
    report_lines.append("")
    ...
    print(f"Summary report saved to: {report_file.absolute()}")
    print("\nKey findings:")
    print(f"- {clean_rate:.1f}% clean file rate")
    print(f"- {total_elements:,} total elements extracted")
    print(f"- {total_orphans} orphan elements need attention")
    print(f"- {total_trash} trash metadata elements detected")
