...
TRASH = re.compile(r"(Field:\s*(Rule-)?Page|ZEQ\.\=1,SEQ=|Text\s+Omitted)", re.I)
...
def check(path):
    ...
    for e in els:
        if TRASH.search(e.get("text","")):
            trash_samples.append(f"{e['class_name']}: {e['text'][:80]}")
...
if __name__ == "__main__":
    for fp in map(pathlib.Path, sys.argv[1:]):
        d, o, t = check(fp)
        print(f"{fp.name}: dup={len(d)}  orphan={len(o)}  trash={len(t)}")