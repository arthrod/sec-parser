...
from json_io import (
    get_normalized_id, build_cross_reference_index, 
...
class CrossReferenceGraph:
    ...
    def add_edge(self, source_id: str, target_id: str, ref_type: str, 
                 confidence: float = 1.0, text_span: str = "", layer: int = 0):
...
class L0DeterministicExtractor:
    ...
    def _canonicalize_reference(self, match: re.Match, pattern: str) -> Optional[str]:
...
class CrossReferenceExtractor:
    ...
    def extract_cross_references(self, elements: List[Any]) -> CrossReferenceGraph:
        ...
        print(f"🔍 Starting cross-reference extraction on {len(elements)} elements...")
        print(f"📊 Built index with {len(index)} referenceable elements")
        
        # Layer 0: Deterministic extraction
        print("⚡ Layer 0: Deterministic regex extraction...")
        l0_refs = self.l0_extractor.extract_references(elements, index)
        print(f"   Found {len(l0_refs)} explicit references")
        
        # Layer 1: Retrieval-assisted (if enabled)
        l1_refs = []
        if self.l1_extractor:
            print("🎯 Layer 1: Retrieval-assisted extraction...")
            l1_refs = self.l1_extractor.extract_references(elements, index)
            print(f"   Found {len(l1_refs)} implicit references")
        ...
        if self.l2_validator and all_refs:
            ...
            print(f"   Validated {len(validated_refs)} references")
            graph.references = validated_refs
        else:
            graph.references = all_refs
        
        print(f"✅ Cross-reference extraction complete: {len(graph.references)} total references")
        return graph
    ...
    def _calculate_coverage_metrics(self, graph: CrossReferenceGraph) -> Dict[str, Any]:
        ...
        elements_with_refs = set(ref.source_id for ref in graph.references)
        
        # Elements that are referenced by others
        referenced_elements = set(ref.target_id for ref in graph.references)
...
def demo_cross_reference_extraction():
    ...
    from json_io import AgreementTitleElement, ArticleElement, SectionElement, ClauseElement
    ...
    print(f"By Layer: {report['references_by_layer']}")
    print(f"By Type: {report['references_by_type']}")
    print(f"Average Confidence: {report['average_confidence']}")
    print(f"Coverage: {report['coverage_metrics']['coverage_percentage']}%")
    
    print("\n🔗 Detailed References:")
    for i, ref in enumerate(graph.references, 1):
        print(f"{i}. {ref.source_id} → {ref.target_id}")
        print(f"   Type: {ref.reference_type}, Confidence: {ref.confidence:.2f}")
        print(f"   Text: '{ref.text_span[:50]}...'")
        print()
