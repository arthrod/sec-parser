...
def create_tree_visualization(tree_data: Dict[str, Any]) -> str:
    ...
    def format_element(element: Dict[str, Any], indent: int = 0, max_content_length: int = 80) -> List[str]:
        ...
        for i, content_elem in enumerate(content_elements[:5]):  # Limit to first 5
        ...
        for i, child in enumerate(children[:3]):  # Limit to first 3
    ...
    if 'cross_references' in tree_data and tree_data['cross_references']:
...
def main():
    ...
    logger.info(f"✅ Generated 1 comprehensive analysis report")
    logger.info(f"✅ Generated 1 index file")
    ...
    logger.info(f"  3. Browse individual visualization files for detailed document analysis")
