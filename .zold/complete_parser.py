...
class UniversalEDGARParser:
    ...
    def parse(self, raw_content: str, filename: Optional[str] = None) -> ParseResult:
        ...
        try:
            ...
            logger.exception(f"Parse failed for {filename}: {e!s}")
        ...
        for i, element in enumerate(elements):
            if self._is_signature_start(element.content):
                ...
                for j in range(i + 1, len(elements)):
                    ...
                    elif (elements[j].type == ElementType.SECTION or
    ...
    def _update_stats(self, filename: str, result: ParseResult) -> None:
...
def test_all_agreements():
    ...
    for result in results.values():
        if result and result.success and result.title:
            pass
